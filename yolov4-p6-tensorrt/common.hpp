...
std::vector<float> getAnchors(std::map<std::string, Weights>& weightMap)
{
    ...
    Weights Yolo_Anchors = weightMap["model.42.anchor_grid"];
    assert(Yolo_Anchors.count == 32);
    int each_yololayer_anchorsnum = Yolo_Anchors.count / 4;
...
IPluginV2Layer* addYoLoLayer(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, IConvolutionLayer* det0, IConvolutionLayer* det1, IConvolutionLayer* det2, IConvolutionLayer* det3)
{
