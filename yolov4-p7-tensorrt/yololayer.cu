...
namespace nvinfer1
{
    YoloLayerPlugin::YoloLayerPlugin(int classCount, int netWidth, int netHeight, int maxOut, const std::vector<Yolo::YoloKernel>& vYoloKernel)
    {
        ...
        mYOLOV4LargeWidth = netWidth;
        mYOLOV4LargeHeight = netHeight;
    ...
    YoloLayerPlugin::YoloLayerPlugin(const void* data, size_t length)
    {
        ...
        read(d, mThreadCount);
        read(d, mKernelCount);
        read(d, mYOLOV4LargeWidth);
        read(d, mYOLOV4LargeHeight);
    ...
    void YoloLayerPlugin::serialize(void* buffer) const
    {
        ...
        write(d, mYOLOV4LargeWidth);
        write(d, mYOLOV4LargeHeight);
    ...
    {
        return sizeof(mClassCount) + sizeof(mThreadCount) + sizeof(mKernelCount) + sizeof(Yolo::YoloKernel) * mYoloKernel.size() + sizeof(mYOLOV4LargeWidth) + sizeof(mYOLOV4LargeHeight) + sizeof(mMaxOutObject);
    }
    ...
    {
        YoloLayerPlugin* p = new YoloLayerPlugin(mClassCount, mYOLOV4LargeWidth, mYOLOV4LargeHeight, mMaxOutObject, mYoloKernel);
    ...
    void YoloLayerPlugin::forwardGpu(const float *const * inputs, float* output, cudaStream_t stream, int batchSize)
    {
        ...
        for (unsigned int i = 0; i < mYoloKernel.size(); ++i)
        {
            ...
            //printf("Net: %d  %d \n", mYOLOV4LargeWidth, mYOLOV4LargeHeight);
            CalDetection << < (yolo.width*yolo.height*batchSize + mThreadCount - 1) / mThreadCount, mThreadCount >> >
                (inputs[i], output, numElem, mYOLOV4LargeWidth, mYOLOV4LargeHeight, mMaxOutObject, yolo.width, yolo.height, (float *)mAnchor[i], mClassCount, outputElem);
        }
    }
    ...
    IPluginV2IOExt* YoloPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc)
    {
        ...
        std::vector<Yolo::YoloKernel> yolo_kernels(5);
        ...
        for (int i = 0; i < fc->nbFields; i++) {
            if (strcmp(fields[i].name, "netdata") == 0) {
                ...
                yolo_kernels[4 - (fields[i].name[8] - '1')] = kernel;
            }
        }
