#ifndef YOLOV4_COMMON_H_
#define YOLOV4_COMMON_H_
...
ILayer *convBlock(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, ITensor &input, int outch, int ksize, int s, int g, std::string lname)
{
    ...
    IPluginV2 *pluginObj = creator->createPlugin(("mish" + lname).c_str(), pluginData);
...
ILayer *bottleneckCSP(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, ITensor &input, int c1, int c2, int n, bool shortcut, int g, float e, std::string lname)
{
    ...
    IScaleLayer *bn1 = addBatchNorm2d(network, weightMap, *cat->getOutput(0), lname + ".bn", 1e-4);
    auto creator = getPluginRegistry()->getPluginCreator("Mish_TRT", "1");
    const PluginFieldCollection *pluginData = creator->getFieldNames();
    IPluginV2 *pluginObj = creator->createPlugin(("mish" + lname).c_str(), pluginData);
    ITensor *inputTensors2[] = {bn1->getOutput(0)};
    auto mish = network->addPluginV2(&inputTensors2[0], 1, *pluginObj);

    auto cv4 = convBlock(network, weightMap, *mish->getOutput(0), c2, 1, 1, 1, lname + ".cv4");
    return cv4;
}

ILayer *bottleneckCSP2(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, ITensor &input, int c1, int c2, int n, bool shortcut, int g, float e, std::string lname)
{
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    int c_ = int(c2);
    auto cv1 = convBlock(network, weightMap, input, c_, 1, 1, 1, lname + ".cv1");

    auto cv2 = network->addConvolutionNd(*cv1->getOutput(0), c_, DimsHW{1, 1}, weightMap[lname + ".cv2.weight"], emptywts);
    ...
    auto cv3 = convBlock(network, weightMap, *mish->getOutput(0), c2, 1, 1, 1, lname + ".cv3");
    return cv3;
}

ILayer *SPPCSP(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, ITensor &input, int c1, int c2, float e, int k1, int k2, int k3, std::string lname)
{
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    auto cv1 = convBlock(network, weightMap, input, c_, 1, 1, 1, lname + ".cv1");
    auto cv2 = network->addConvolutionNd(input, c_, DimsHW{1, 1}, weightMap[lname + ".cv2.weight"], emptywts);
    auto cv3 = convBlock(network, weightMap, *cv1->getOutput(0), c_, 3, 1, 1, lname + ".cv3");
    auto cv4 = convBlock(network, weightMap, *cv3->getOutput(0), c_, 1, 1, 1, lname + ".cv4");

    auto pool1 = network->addPoolingNd(*cv4->getOutput(0), PoolingType::kMAX, DimsHW{k1, k1});
    pool1->setPaddingNd(DimsHW{k1 / 2, k1 / 2});
    pool1->setStrideNd(DimsHW{1, 1});
    auto pool2 = network->addPoolingNd(*cv4->getOutput(0), PoolingType::kMAX, DimsHW{k2, k2});
    pool2->setPaddingNd(DimsHW{k2 / 2, k2 / 2});
    pool2->setStrideNd(DimsHW{1, 1});
    auto pool3 = network->addPoolingNd(*cv4->getOutput(0), PoolingType::kMAX, DimsHW{k3, k3});
    pool3->setPaddingNd(DimsHW{k3 / 2, k3 / 2});
    pool3->setStrideNd(DimsHW{1, 1});

    ITensor *inputTensors1[] = {cv4->getOutput(0), pool1->getOutput(0), pool2->getOutput(0), pool3->getOutput(0)};
    auto cat1 = network->addConcatenation(inputTensors1, 4);
    auto cv5 = convBlock(network, weightMap, *cat1->getOutput(0), c_, 1, 1, 1, lname + ".cv5");
    auto cv6 = convBlock(network, weightMap, *cv5->getOutput(0), c_, 3, 1, 1, lname + ".cv6");

    ITensor *inputTensors2[] = {cv6->getOutput(0), cv2->getOutput(0)};
    auto cat2 = network->addConcatenation(inputTensors2, 2);

    IScaleLayer *bn1 = addBatchNorm2d(network, weightMap, *cat2->getOutput(0), lname + ".bn", 1e-4);
    auto creator = getPluginRegistry()->getPluginCreator("Mish_TRT", "1");
    const PluginFieldCollection *pluginData = creator->getFieldNames();
    IPluginV2 *pluginObj = creator->createPlugin(("mish" + lname).c_str(), pluginData);
    ITensor *inputTensors3[] = {bn1->getOutput(0)};
    auto mish = network->addPluginV2(&inputTensors3[0], 1, *pluginObj);

    auto cv7 = convBlock(network, weightMap, *mish->getOutput(0), c2, 1, 1, 1, lname + ".cv7");
    return cv7;
}
...
std::vector<float> getAnchors(std::map<std::string, Weights>& weightMap)
{
    ...
    Weights Yolo_Anchors = weightMap["model.53.anchor_grid"];
    assert(Yolo_Anchors.count == 40);
    int each_yololayer_anchorsnum = Yolo_Anchors.count / 5;
    ...
    for (int i = 0; i < Yolo_Anchors.count; i++)
    {
        ...
        if ((i >= (3 * each_yololayer_anchorsnum)) && (i < (4 * each_yololayer_anchorsnum)))
        {
            anchors_yolo.push_back(const_cast<float*>(tempAnchors)[i]);
        }
        if (i >= (4 * each_yololayer_anchorsnum))
        {
            anchors_yolo.push_back(const_cast<float*>(tempAnchors)[i]);
        }
    }
...
IPluginV2Layer* addYoLoLayer(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, IConvolutionLayer* det0, IConvolutionLayer* det1, IConvolutionLayer* det2, IConvolutionLayer* det3, IConvolutionLayer* det4)
{
    auto creator = getPluginRegistry()->getPluginCreator("YoloLayer_TRT", "1");
    std::vector<float> anchors_yolo = getAnchors(weightMap);
    PluginField pluginMultidata[6];
    int NetData[4];
    ...
    ITensor* inputTensors_yolo[] = { det4->getOutput(0), det3->getOutput(0), det2->getOutput(0), det1->getOutput(0), det0->getOutput(0) };
    auto yolo = network->addPluginV2(inputTensors_yolo, 5, *pluginObj);
    return yolo;
}
#endif
